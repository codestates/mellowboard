...
module.exports = {
    get: async (req, res) => {
        ...
        const total = parseInt(commentsCount / size, 10) + commentsCount % size ? 1 : 0
        ...
        return res.json({
            ...
            comments: comments.map(el => {
                ...
                // 자신이 작성한 댓글인지 체크
                if(comment.user_id === userId) comment.isMine = true;
    ...
    patch: async (req, res) => {
         ...
         // 본인의 댓글인지 확인
         if(comment.user_id !== res.locals.userId) return res.status(401).json({
